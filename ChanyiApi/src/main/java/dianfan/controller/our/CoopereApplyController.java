/**  
* @Title: CoopereApplyController.java
...
/**
 * @ClassName CoopereApplyController
...
@Controller
@RequestMapping(value = "/coopere")
public class CoopereApplyController {
	...
	@SystemControllerLog(method = "coopereApply", logtype = ConstantIF.LOG_TYPE_1, description = "申请成为运营服务商")
	@ApiOperation(value = "申请成为运营服务商", httpMethod = "POST", notes = "申请成为运营服务商", response = ResultBean.class)
	@RequestMapping(value = "/coopereApply", method = RequestMethod.POST)
	@ResponseBody
	public ResultBean coopereApply(
			...
			@ApiParam(value = "地址") @RequestParam(value = "applyaddr", required = false) String applyaddr,
			...
			@ApiParam(value = "营业执照") @RequestParam(value = "businessLicense", required = true) String businessLicense,
		...
		//限制申请人姓名位数
		int applen = applyname.length();
		if (applen>25) {
			return new ResultBean("2021",ResultApiMsg.C_2021);
		}
		//限制地址位数
		int addrlen = applyaddr.length();
		if (addrlen>64) {
			return new ResultBean("2022",ResultApiMsg.C_2022);
		}
		//限制真实姓名位数
		...
		if (tokens != null) {
			...
			if (StringUtils.isNotEmpty(userid)) {
				int coopereStatus = coopereApplyService.getCooperationApply(userid);
				//int coopereByPhoneStatus = coopereApplyService.getCooperationApplyByPhone(applyphonenum);
				if (coopereStatus > 0) {
					coopereApplyService.updateCoopereApply(userid, applyname, applyphonenum, applyaddr, realname, idcardno, idcardfont, idcardback, idcardvaliddate, handleidcard,cityCode,businessLicense,companyName);
				...
				if (uim != null) {
					// 此处调用业务逻辑层
					ResultBean rb = coopereApplyService.addCoopereApply(userid, applyname, applyphonenum, applyaddr, realname, idcardno, idcardfont,
							idcardback, idcardvaliddate, handleidcard,
							ConstantIF.APPLY_STATUS3,cityCode,businessLicense,companyName);
					return rb;
				}
			}
		}
