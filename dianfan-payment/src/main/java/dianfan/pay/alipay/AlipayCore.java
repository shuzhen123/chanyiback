...
import dianfan.entities.alipay.AlipayRefundBean;
...
public class AlipayCore {
	...
	public AlipayCore(PublicPayParam base) {
		this.client = new DefaultAlipayClient("https://openapi.alipay.com/gateway.do", base.getApp_id(), // 商户appid
				base.getApp_private_key(), // 商户私钥
				AlipayConstants.FORMAT_JSON, AlipayConstants.CHARSET_UTF8, base.getAlipay_public_key(), // 支付宝公钥
	 ...
	 *            支付宝支付请求公共参数
	 */
	public AlipayCore(String request_url, PublicPayParam base) {
		new DefaultAlipayClient(request_url, base.getApp_id(), base.getApp_private_key(), AlipayConstants.FORMAT_JSON,
				AlipayConstants.CHARSET_UTF8, base.getAlipay_public_key(), AlipayConstants.SIGN_TYPE_RSA2);
	}
	 ...
	 * 
	 *        更新时间：2018年6月6日 上午11:07:23 更新备注：BeanMap 去null修改
	 */
	 ...
	 * @throws IOException
	 * @throws:
	 * @time: 2018年7月9日 下午3:09:11
	 */
	public String refundQuery(AlipayRefundBean queryData) throws AlipayApiException, IOException {
		...
		String bizContent = mapper.writeValueAsString(query_data);
		req.setBizContent(bizContent);
		AlipayTradeRefundResponse response = client.execute(req);
	 ...
	 * @time: 2018年7月9日 下午3:09:11
	 */
	public String refund(AlipayRefundBean queryData) throws AlipayApiException, IOException {
		AlipayTradeRefundRequest req = new AlipayTradeRefundRequest();
		Map<String, String> query_data = new HashMap<>();
		// 业务参数转换成map
		BeanMap map = new BeanMap(queryData);
		...
		req.setBizContent(bizContent);
		AlipayTradeRefundResponse response = client.execute(req);
		return response.getBody();// 失败会返回错误信息
	}
}
