...
public class WxPayCore {
	 ...
	 */
	public Map<String, String> unifiedOrder(UnifiedorderBean order, String payType, SignType signType)
			throws Exception {
		...
		if (StringUtils.equals("SUCCESS", StringUtils.upperCase(wpc.get("return_code")))
			...
			if (StringUtils.equals(payType, WxTradeType.JSAPI)) {
				...
				signMap.put("signType",
						(SignType.MD5.equals(signType)) ? WXPayConstants.MD5 : WXPayConstants.HMACSHA256);
	 ...
	 * @Description: 微信订单查询
	 * @param orderno
	 *            微信的订单号 或 商户系统内部订单号（二选一，微信的订单号，建议优先使用）
	 * @param signType
	 *            签名类型，默认为MD5，支持HMAC-SHA256和MD5
	 * @return
	 * @throws Exception
	 ...
	 * @time: 2018年5月29日 下午5:15:22
	 */
	public Map<String, String> refundQuery(WxOrderRefundBean wxOrderRefundQuery) throws Exception {
		return refundQuery(wxOrderRefundQuery, SignType.MD5);
	 ...
	 * @return
	 * @throws:
	 * @time: 2018年7月9日 下午3:57:02
	 */
	private Map<String, String> refundQuery(WxOrderRefundBean orderno, SignType signType) throws Exception {
		log.debug("微信订单查询订单号：" + orderno);
		WXPay wxpay;
		if (SignType.MD5.equals(signType)) {
			wxpay = new WXPay(conf, SignType.MD5, this.useSandbox);
	 ...
	 * @time: 2018年5月29日 下午5:15:22
	 */
	public Map<String, String> refund(WxOrderRefundBean orderRefund) throws Exception {
		return refund(orderRefund, SignType.MD5);
	 ...
	 * @time: 2018年7月9日 下午3:57:02
	 */
	private Map<String, String> refund(WxOrderRefundBean orderno, SignType signType) throws Exception {
		log.debug("微信订单查询订单号：" + orderno);
		WXPay wxpay;
		if (SignType.MD5.equals(signType)) {
			wxpay = new WXPay(conf, SignType.MD5, this.useSandbox);
		...
		Map<String, String> refund = wxpay.refund(reqData);
		return refund;
	}
}
